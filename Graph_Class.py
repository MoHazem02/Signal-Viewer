...
class Graph:
    ...
    def Scroll_Signal(self, Scrolling_Coordinates_Value):
        ...
        index = min(int(Scrolling_Coordinates_Value / self.UI_Window.Horiz_ScrollBar_Top.maximum() * len(
            self.CHANNELS[self.Current_Channel - 1].Signal.X_Coordinates)),
                    len(self.CHANNELS[self.Current_Channel - 1].Signal.X_Coordinates) - 1)
        ...
        self.CHANNELS[self.Current_Channel - 1].Signal.Graph_Widget.getViewBox().setXRange(
            max(self.CHANNELS[self.Current_Channel - 1].Signal.X_Coordinates[0: index + 1]) - 100,
            max(self.CHANNELS[self.Current_Channel - 1].Signal.X_Coordinates[0: index + 1]))
